...
class ApplicantProfileVC: UIViewController,ResponseProtcol {
    
    //MARK: - IBOutlets
    
    @IBOutlet var imgUser_pop: UIImageView!
    @IBOutlet var lblUserName_pop: UILabel!
    @IBOutlet var ratingView_pop: FloatRatingView!
    @IBOutlet var lblMobile_pop: UILabel!
    @IBOutlet var lblEmail_pop: UILabel!
    @IBOutlet var lblDate_pop: UILabel!
    @IBOutlet var lblTime_pop: UILabel!
    ...
    func getProfileDetailsAPI()
    {
        ...
        let param : [String:String] = [KuserID:applicant_ID]
        print(param)
    ...
    func Response(Resposnedic : [String:Any])
    {
        ...
        else if Resposnedic["status"] as! NSInteger == 200
        {
            ...
            if(api == KacceptRejectApplicant)
            {
                if(msg == KAccepted)
                {
                    view_Success.isHidden = false
                   // lblDate_pop.text = "\(Resposnedic[Kjob_date]!)".ChangeUTCDateStrToCurrent()
                  //  let str = "\(Resposnedic[Kjob_time]!)".ChangeUTCTimeStrToCurrent()
                   // lblTime_pop.text = str + Konwards
               // MethodForDate
                 if let jobDate = Resposnedic[Kjob_date_time]
                  {
                     let  completeDateStr = jobDate as! String
                    lblDate_pop.text = completeDateStr.MethodForDate()
                    lblTime_pop.text = completeDateStr.MethodForSuccessTime() +  Konwards
                   }
                }
                else
                {
                    let alertController = UIAlertController(title: "", message: KRejectedSuccessfully, preferredStyle: .alert)
    ...
    func LoadData()
    {
        ...
        if let  mobile = dic_ApplicantProfile[KMobile]
        {
            lblMobile_pop.text = mobile as? String
        }
        if let  email = dic_ApplicantProfile[KEmail]
        {
            lblEmail_pop.text = email as? String
            
        }
        if let  userName = dic_ApplicantProfile[KName]
        {
            lblUserName.text = userName as? String
            lblUserName_pop.text = userName as? String
        }
        if let shift = dic_ApplicantProfile[Kshifts]
        {
            lblShift_value.text = "\(shift)"             //lblShift_value.text = "4"
        }
        ...
        if let imageURL = dic_ApplicantProfile[KImage]
        {
            if let url = URL.init(string: imageURL as! String) {
                ...
                imgUser_pop.setShowActivityIndicator(true)
                imgUser_pop.setIndicatorStyle(.gray)
                imgUser_pop.sd_setImage(with: url, placeholderImage: UIImage(named: Kimg))
            }
        }
    ...
    func setRating(value:Float)
    {
        ...
        self.ratingView_pop.contentMode = UIViewContentMode.scaleAspectFit
        self.ratingView_pop.rating = value
        self.ratingView_pop.editable = false
        self.ratingView_pop.halfRatings = false
        self.ratingView_pop.floatRatings = true
    }
    ...
    {
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: KGetJobs), object: nil)
    ...
    @IBAction func Decline_Action(_ sender: Any)
    {
        MBProgressHUD.showAdded(to: self.view, animated: true)
        let param : [String:String] = [KuserID:applicant_ID,Kjobid:job_id,KStatus:"R"]
        print(param)
        let obj = ServerRequest()
        obj.delegate = self
        let url = kBaseUrl + KacceptRejectApplicant
        if Reach.isConnectedToNetwork() == true
        {
            obj.postMethod(urlStr: url, parameters: param)
        }
    ...
    {
        MBProgressHUD.showAdded(to: self.view, animated: true)
        let param : [String:String] = [KuserID:applicant_ID,Kjobid:job_id,KStatus:"A"]
        print(param)
        let obj = ServerRequest()
        obj.delegate = self
        let url = kBaseUrl + KacceptRejectApplicant
    ...
    @IBAction func BtnLivesAction2(_ sender: Any) {
        let tutorialVC = UIStoryboard(name: KTutorial, bundle: nil).instantiateViewController(withIdentifier: KTutorialViewController) as! TutorialViewController
        self.navigationController?.pushViewController(tutorialVC, animated: true)
    }
